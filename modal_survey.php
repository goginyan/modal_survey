...
define( 'GRID_ITEMS' , '' );
define( 'MODAL_SURVEY_VERSION' , '1.9.9.7' );
define( 'MSDIRS' , '/' );
...
http://pantherius.com/modal-survey' );
		...
		**/
		...
		public function survey_answers_shortcodes( $atts ) {
			...
			if ( ! empty( $current_user->user_login ) && $args[ 'uid' ] != "false" && ! isset( $_COOKIE[ 'ms-uid' ] ) && $atts[ 'session' ] == "last" && $args[ 'uid' ] == "true" ) {
									...
									foreach( $fr[ 'datas' ] as $frd ) {
										...
										if ( isset( $acat[ 1 ][ 0 ] ) ) {
											...
											foreach ( $acat_list as $acal ) {
												if ( isset( $acal ) ) {
													if ( ! empty( $acal ) && ! is_numeric( $acal ) && $frd[ 'selected' ] == "true" && $frd[ 'score' ] > 0  ) {
					...
					//end - remove inactive answers					
						if ( isset( $qoptions[ 3 ] ) && ( $qoptions[ 3 ] == 1 ) ) {
							...
							if ( ( $key1 + 1 ) == $args[ 'qid' ] && $args[ 'data' ] != 'full' ) {
								...
								if ( $args[ 'filter' ] != '' ) {
									$sdatas = $this->filter_result( $args[ 'filter' ], $sdatas );
								}
								$msplugininit_answer_array[ $args[ 'id' ] . '-' . $unique_key ] = array( 
									"style" => array( 
										"style" => $args[ 'style' ],
			...
			if ( $args[ 'percentage' ] == "true" && $args[ 'data' ] == "question" ) {
				...
				foreach( $sdatas[ 0 ] as $sdkey=>$sd ) {
					$sdatas[ 0 ][ $sdkey ][ "count" ] = round( ( ( $sd[ 'count' ] / $tqallvotes ) * 100 ), 2 );
				}
			}
